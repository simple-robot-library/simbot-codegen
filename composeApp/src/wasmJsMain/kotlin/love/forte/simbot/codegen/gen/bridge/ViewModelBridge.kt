...
class ViewModelBridge(
    ...
    suspend fun generateProject(viewModel: GradleProjectViewModel): JSZip {
        ...
        val context = createGenerationContext {
            ...
            // 如果选择Kotlin语言，隐式添加Kotlin相关依赖
            if (language is ProgrammingLanguage.Kotlin) {
                dependencies.add(KOTLIN_REFLECT.toDependency())
                dependencies.add(JACKSON_MODULE_KOTLIN.toDependency())
            }
            ...
            // 添加选中的 SpringBoot 组件依赖
            for (springComponent in viewModel.selectedSpringComponentMask.components()) {
                val springDependency = GradleCatalogVersionDependency(
                    dependencyName = springComponent.depName,
                    group = springComponent.group,
                    name = springComponent.artifactId,
                    version = null, // Spring Boot管理版本
                    libRefPath = springComponent.libPath
                )
                dependencies.add(springDependency.toDependency())
            }
        }
